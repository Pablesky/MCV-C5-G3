...
def train(model_image, model_text, loss_func, mining_func, device, train_loader, optimizer_image, optimizer_text, epoch):
    ...
    for batch_idx, (images, captions, text_features, ids) in enumerate(train_loader):
        ...
        indices_tuple = mining_func(
            ...
            labels = ids, 
            ref_emb = text_features, 
            ref_labels = ids,
            )

        #loss = loss_func(embeddings, labels, indices_tuple)
        loss = loss_func(embeddings = image_features, indices_tuple=indices_tuple, ref_emb=text_features, labels = ids)
        loss.backward()
...
def caller(config = None):
    with wandb.init(config=config):
        ...
        wandb.run.name = f'./model_bert_a2_10_{margin}_{lr}_{batch_size}'
        ...
        torch.save(model_image.state_dict(), f'./model_image_bert_a2_10.pth')
        torch.save(model_text.state_dict(), f'./model_text_bert_a_10.pth')

        generate_embeddings(f'./model_image_bert_a2_10.pth', f'./model_text_bert_a2_10.pth', '2_10_img2txt')
